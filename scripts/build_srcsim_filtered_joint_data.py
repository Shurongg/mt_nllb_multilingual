...
def topk_mean_scores(sim_matrix, k=5):
    k = min(k, sim_matrix.shape[1])
    part = np.partition(sim_matrix, -k, axis=1)[:, -k:]
...
def main():
    ...
    java_texts = [r["src_text"] for r in jav]
    indo_texts = [r["src_text"] for r in ind_quality]
    ...
    scores = topk_mean_scores(sims, k=args.top_k)

    scored = []
    for r, score in zip(ind_quality, scores):
        rr = dict(r)
        rr["selection_method"] = f"source_char_tfidf_top{args.top_k}_mean_to_java_train"
        rr["selection_score"] = float(score)
        scored.append(rr)

    scored.sort(key=lambda x: x["selection_score"], reverse=True)

    ranked_path = out_dir / f"ind_eng_srcsim_top{args.top_k}_candidates.jsonl"
    write_jsonl(ranked_path, scored)
    print("Wrote ranked candidates:", ranked_path)
    ...
    for n in args.top_ns:
        ...
        out_path = out_dir / f"train_javx2_ind{n}_srcsim.jsonl"
